...
function fillSeasonTable(bodyId,arr,speakResolver){
  ...
  arr.forEach((row,idx)=>{
    ...
    tr.innerHTML = `<td class="p-2 font-semibold">${row[0]}</td><td class="p-2">${row[1]}</td><td class="p-2">${row[2]}</td><td class="p-2 text-center no-print">${ttsBtn(bnText, enText)}</td>`;
    tbody.appendChild(tr);
  });
}
fillSeasonTable('t5BodyBn', bnSeasons, row => [row[0], row[2]]);
fillSeasonTable('t5BodyEn', enSeasons, row => [row[1], row[0]]);
fillSeasonTable('t5BodyAr', arSeasons, row => [row[2], row[1]]);
